import{FirebaseError,getDefaultEmulatorHostnameAndPort,isCloudWorkstation,pingServer,deepEqual,createMockUserToken,getModularInstance}from"@firebase/util";import{Logger,LogLevel}from"@firebase/logger";import{Integer}from"@firebase/webchannel-wrapper/bloom-blob";import{_isFirebaseServerApp,_getProvider,getApp,_removeServiceInstance}from"@firebase/app";
/**
 * @license
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const d=new Logger("@firebase/firestore");function setLogLevel(e){d.setLogLevel(e)}function __PRIVATE_logDebug(e,...t){if(d.logLevel<=LogLevel.DEBUG){const r=t.map(__PRIVATE_argToString);d.debug(`Firestore (${f}): ${e}`,...r)}}function __PRIVATE_logError(e,...t){if(d.logLevel<=LogLevel.ERROR){const r=t.map(__PRIVATE_argToString);d.error(`Firestore (${f}): ${e}`,...r)}}function __PRIVATE_logWarn(e,...t){if(d.logLevel<=LogLevel.WARN){const r=t.map(__PRIVATE_argToString);d.warn(`Firestore (${f}): ${e}`,...r)}}function __PRIVATE_argToString(e){if("string"==typeof e)return e;try{return function(e){return JSON.stringify(e)}(e)}catch(t){return e}}
/**
 * @license
 * Copyright 2017 Google LLC
 *
