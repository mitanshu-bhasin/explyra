...
function redirectCompanyRoute(targetPage, companyId) {
    if (window.ExplyraTenant && companyId) {
        window.ExplyraTenant.redirectToTenantPath(`attendance/company/${targetPage}`, companyId);
        return;
    }
...
function initAuth(moduleType, isLoginPage = false) {
    ...
    if (isLoginPage) {
        if (session) {
            if (moduleType === 'company') redirectCompanyRoute('dashboard.html', session.companyId);
            else window.location.href = 'dashboard.html';
        }
    } else {
        if (!session) {
            if (moduleType === 'company') {
                const companyFromPath = window.ExplyraTenant?.getCompanyIdFromPath() || null;
                redirectCompanyRoute('login.html', companyFromPath);
            } else {
            ...
            .then(function(userCredential) {
                ...
                return window.db.collection('companies').doc(companyId).set({
                    ...
                    return window.db.collection('users').doc(userCredential.user.uid).set({
                        ...
                        companyId: companyId,
                    redirectCompanyRoute('login.html', companyId);
...
async function handleCompanyLogin(e) {
    ...
    if (typeof window.auth !== 'undefined' && typeof window.db !== 'undefined') {
        ...
        try {
            ...
            if (urlCompanyId && account.companyId && urlCompanyId !== account.companyId) {
                alert("This workspace link belongs to another company.");
                await window.auth.signOut();
                btn.innerHTML = originalBtnContent;
                btn.disabled = false;
                return;
            ...
            saveSession('company_session', {
                companyId: urlCompanyId || account.companyId || 'EXPLYRA_INTERNAL',
                role: account.role || 'Admin',
                userId: account.name || account.email.split('@')[0]
            });
            redirectCompanyRoute('dashboard.html', urlCompanyId || account.companyId);
        ...
        redirectCompanyRoute('dashboard.html', account.companyId);
    }
}
