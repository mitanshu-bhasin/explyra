...
- [ ] **Organization Schema**: Ensure all subdomains (Netlify, etc.) have the `Organization` schema pointing to `https://explyra.me/` (Primary Domain) and `https://ipecconsulting.org/` (Primary Entity) as the key entities.
...
- [ ] **Canonical Tags**: On all entry points, ensure `<link rel="canonical" href="https://explyra.me/">` is correctly set.
- [ ] **Link Consolidation**: If you have multiple landing pages, ensure they all point to `https://explyra.me/` as the primary home for the Expense Manager.

## 3. Google Business Profile (GBP)
- [ ] **Featured Products**: Add "IPEC Expense Manager" as a 'Product' in your Google Business Profile dashboard. This helps it appear in the Knowledge Panel. Link it to `https://explyra.me/`.
