/**
 * Explyra Suite Status Page Logic
 */
...
function renderServices(groups) {
    ...
    const productContainer = document.getElementById('product-systems');
    const infraContainer = document.getElementById('infra-services');

    if (!coreContainer || !productContainer || !infraContainer) return;
    ...
    groups.forEach(group => {
        let container;
        if (group.name.includes('Core')) container = coreContainer;
        else if (group.name.includes('Product')) container = productContainer;
        else if (group.name.includes('Infrastructure')) container = infraContainer;

        if (container) {
            group.services.forEach(service => {
                const card = document.createElement('status-card');
                card.setAttribute('name', service.name);
                card.setAttribute('status', service.status);
                card.setAttribute('last-checked', new Date(service.last_checked).toLocaleString());
