...
function crmRedirect(target, companyId) {
    if (window.ExplyraTenant) {
        window.ExplyraTenant.redirectToTenantPath(target, companyId);
        return;
    }
...
async function initializeCrm(user) {
    try {
        ...
        if (userData.role !== 'MANAGER' && userData.role !== 'SENIOR_MANAGER' && userData.role !== 'ADMIN' && userData.role !== 'MAIN_ADMIN') {
            ...
            crmRedirect('emp.html', userData.companyId);
            return;
        }
        ...
        if (urlCompanyId && userData.companyId && urlCompanyId !== userData.companyId) {
            throw new Error("Company URL does not match your account workspace.");
        }

        window.userData = { ...userData, uid: user.uid };
        window.companyId = urlCompanyId || userData.companyId;
